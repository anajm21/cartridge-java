    ...
    stages {
		...
		stage('Provision & Deploy to Test') {
            steps {
					...
					sh "docker run -it -d --name tomcat -p 8888:8080 -v tomcat:/usr/local/tomcat tomcat:9.0"
					/*sh ''' 
		...
		stage('Performance Test') {
            steps {	
				...
				archiveArtifacts '/workspace/java-cartridge/target/jmeter/results/20171121-petclinic_test_plan.jtl'
				gatlingArchive()
